...
def get_input_feats(input_list, models):
    ...
    for input, model in zip(input_list, models):
        dif = torch.cat([torch.zeros(*input.shape[:3], 1).zero_(), input[:, :, :, 1:] - input[:, :, :, 0:-1]], dim=-1)
...
class ADASGN(nn.Module):
    def __init__(self, num_classes, num_joints, seg, bias=True, dim=256, tau=5., policy_kernel=3,
                 ...
                 pre_trains=None, tau_type='cos', init_type='fix', init_num=5, gcn_types=[]):
        ...
        self.spa_nets = nn.ModuleList([SpatialNet(num_joint, bias, dim, gcn_type)
                                       for num_joint in num_joints for gcn_type in gcn_types])
    ...
    def get_gflops_table(self):
        ...
        self.gflops_table = {}
        self.gflops_table['spanet'] = [get_model_complexity_info(m, (3, j, self.seg), input_constructor=input_constructor, **kwards)[0] / 1e9 for
    ...
    def test(self, input):
        ...
        diff_list = [torch.cat([torch.zeros(*input.shape[:3], 1).zero_(), input[:, :, :, 1:] - input[:, :, :, 0:-1]], dim=-1) for input in input_list]
        ...
        for i in range(bs * m * s):
            ...
            for j in range(step):
                ...
                if a == 0:
                    ...
                    feats.append(self.spa_nets[a](input_list[a][i:i + 1, :, :, j:j + 1], diff_list[a][i:i + 1, :, :, j:j + 1]))  # b c 1 1
