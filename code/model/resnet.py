...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=3, drop=0.5, out_fea=False):
        ...
        if block.expansion == 1:
            self.fea_fc = lambda x: x
        else:
            self.fea_fc = nn.Linear(512 * block.expansion, 512)
        if not out_fea:
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), 512 * self.block.expansion, -1)
        x = torch.mean(x, 2)
        x = self.fea_fc(x)
        if not self.out_fea:
            x = self.drop(x)
...
def resnet50(pretrained=False, ignore_weights=['fc'], **kwargs):
