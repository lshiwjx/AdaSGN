...
def align_frames(skes_joints, frames_cnt):
    ...
    aligned_skes_joints = np.zeros((num_skes, max_num_frames, 150), dtype=np.float32)  # n t m v c
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path, ske_names):
    ...
    skes_joints = skes_joints.reshape([-1, 300, 2, 25, 3]).transpose(0, 4, 1, 3, 2)  # n t m v c -> n c t v m

    if not os.path.exists('{}/{}'.format(save_path, evaluation)):
        os.makedirs('{}/{}'.format(save_path, evaluation))

    np.save('{}/{}/{}_data.npy'.format(save_path, evaluation, 'train'), skes_joints[train_indices])
    np.save('{}/{}/{}_data.npy'.format(save_path, evaluation, 'test'), skes_joints[test_indices])
    with open('{}/{}/{}_label.pkl'.format(save_path, evaluation, 'train'), 'wb') as f:
        pickle.dump((ske_names[train_indices], train_labels), f)
    with open('{}/{}/{}_label.pkl'.format(save_path, evaluation, 'test'), 'wb') as f:
        pickle.dump((ske_names[test_indices], test_labels), f)
...
if __name__ == '__main__':
    ...
    skes_name = open(skes_name_file, 'r').readlines()
    skes_name = np.array([x.strip() for x in skes_name])
    ...
    skes_joints = align_frames(skes_joints, frames_cnt)  # aligned to the same frame length n t v c

    evaluations = ['CS', 'CV']
    for evaluation in evaluations:
        split_dataset(skes_joints, label, performer, camera, evaluation, save_path, skes_name)
